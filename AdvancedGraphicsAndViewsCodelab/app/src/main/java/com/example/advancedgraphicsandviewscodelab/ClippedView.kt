...
class ClippedView @JvmOverloads constructor(
    ...
    private val mRectInset = resources.getDimensionPixelSize(R.dimen.rectInset)
    private val mSmallRectOffset = resources.getDimensionPixelSize(R.dimen.smallRectOffset)
    ...
    private val mTextSize = resources.getDimensionPixelSize(R.dimen.textSize)
    ...
    private val mTextRow = mRowFour + (1.5 * mClipRectBottom)
    ...
    init {
        ...
        mRectF = RectF(
            Rect(
                mRectInset, mRectInset,
                mClipRectRight.toInt() - mRectInset, mClipRectBottom.toInt() - mRectInset
            )
        )
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.translate(mColumnnTwo, mRowOne.toFloat())
        // Use the subtraction of two clipping rectangles to create a frame.
        canvas.clipRect(
            2 * mRectInset, 2 * mRectInset,
            mClipRectRight.toInt() - 2 * mRectInset, mClipRectBottom.toInt() - 2 * mRectInset
        )
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            canvas.clipRect(
                (4 * mRectInset).toFloat(), (4 * mRectInset).toFloat(),
        ...
        canvas.save()
        canvas.translate(mColumnOne.toFloat(), mRowTwo)
        ...
        canvas.save()
        canvas.translate(mColumnOne.toFloat(), mRowThree)
        mPath.rewind()
        mPath.addCircle(
        ...
        mPath.rewind()
        mPath.addRoundRect(
            mRectF, mClipRectRight / 4,
            mClipRectRight / 4, Path.Direction.CCW
        )
        canvas.clipPath(mPath)
        drawClippedRectangle(canvas)
        canvas.restore()
        ...
        canvas.save()
        // Move the origin to the right for the next rectangle.
        canvas.translate(mColumnOne.toFloat(), mRowFour)
        canvas.clipRect(
            2 * mRectInset.toFloat(), 2 * mRectInset.toFloat(),
        ...
        canvas.translate(mColumnnTwo, mTextRow.toFloat())
        // Draw text.
        canvas.drawText(
